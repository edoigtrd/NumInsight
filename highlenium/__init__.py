...
def get_geckodriver():
    ...
    if not os.path.isfile("geckodriver"):
        ...
        if os.name == "posix":
            ...
            r = requests.get(
                "https://github.com/mozilla/geckodriver/releases/download/v0.34.0/geckodriver-v0.34.0-win-aarch64.zip"
            )
    ...
    if DRIVER is None:
        if type is None:
            ...
            service = ChromeService(
                executable_path=get_chromedriver(), options=chrome_options
            )
            DRIVER = webdriver.Chrome(service=service)
            DRIVER.__setattr__(
                "find_element_by_css_selector",
                lambda selector: find_element_by_css_selector(DRIVER, selector),
            )
        elif type == "firefox":
